import type { CompTextToken } from './types'

/**
 * Tokenizes raw input into CompText tokens
 */
export function tokenize(input: string): CompTextToken[] {
  const tokens: CompTextToken[] = []
  let position = 0

  const patterns: Array<{ type: CompTextToken['type'], regex: RegExp }> = [
  ...
  while (position < input.length) {
    ...
    if (ws) { position += ws[0].length; continue }
