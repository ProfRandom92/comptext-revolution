import { ABBREVIATIONS, FILLER_WORDS, PHRASE_COLLAPSES, ARTICLES, VOWELS, } from './dictionary.js';
// Level 1: Normalize whitespace + deduplicate lines
export function applyLevel1(text) {
    return text
        .split('\n')
...
export function applyLevel2(text) {
...
export function applyLevel3(text) {
...
export function applyLevel4(text) {
...
export function applyLevel5(text) {
    let result = applyLevel4(text);
    const words = result.split(/(\s+)/);
    result = words
        .map(word => {
        if (/^\s+$/.test(word) || word.length < 4)
            return word;
